  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-border bg-sidebar">
      ...
      <div className="flex-1 overflow-y-auto">
        {docs.length === 0 && (
          <p className="px-3 py-6 text-center text-xs text-fg-3">
            No documents yet. Use <span className="text-fg-2">+</span> above, or ask the agent to write one.
          </p>
        )}
        {docs.map((doc) => {
            ...
            >
              {editing === doc.id ? (
                ...
                <button
                  ...
                  className={`w-full truncate rounded px-2 py-1 text-left text-sm hover:bg-row-hover ${selected ? 'text-fg-1' : 'text-fg-2'}`}
                >
                  {doc.title}
                </button>
              )}
      ...
      <footer className="flex items-center gap-2 border-t border-border px-3 py-2">
        ...
        <button type="button" onClick={onSignOut} className="rounded px-1.5 py-0.5 text-xs text-fg-3 hover:bg-row-hover hover:text-fg-1">
          Sign out
        </button>
      </footer>
    </aside>
