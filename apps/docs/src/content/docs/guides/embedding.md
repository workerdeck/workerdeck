...
| `renderApproval` | Hands a pending approval to your own card. Called for every approval that is not an `AskUserQuestion` — the question form keeps its own path — with the request and the same `approve` / `deny` resolvers the built-in prompt uses; whatever it returns is drawn in place of the permission prompt. Return `undefined` and that request keeps the default card, so a host draws the one tool it has a real review screen for (a profile the agent wants to save, rendered as the form the reviewer already knows) without owning the rest. |
