...
| `approvalPrompts` | `Record<toolName, ComponentType<ApprovalPromptProps>>` - your own card for the tools you have a real review screen for (a profile the agent wants to save, drawn as the form the reviewer already knows), the built-in prompt for everything else. Looked up **before** the theme split, so an entry draws in `'cards'` and `'terminal'` alike; under `'terminal'` it is mounted on the terminal surface, so read `useTranscriptVariant()` or compose `TerminalPermissionPrompt` if a box on the cell grid is wrong. Entries override the built-in `AskUserQuestion` and `ExitPlanMode` cards too, so claim those names only if you mean to. Your component gets `request` plus the same `onApprove` / `onDeny` the built-ins take - which is what lets you mount `PermissionPrompt` verbatim for an input you decline to draw. Must be a **stable component type** (module scope): an inline arrow is a new type every render and remounts the card, losing whatever the reviewer typed. |
...
| `affordances` | Terminal theme only: the hover fill and the hover-revealed action buttons (bookmark, copy, enter shell) - the things a real terminal cannot do. `false` for none; each costs no layout, so off is the pure article rather than a degraded mode. `{ labels: true }` draws each button with its name beside the icon. |
...
| `unseen` | Catch-up mode: pass `{ itemCount, since }` - the watermark from when the reader last had this session open - and the panel marks the boundary, fades what was already read and offers a "N new rows since you were last here - jump / dismiss" bar. Omit it and none of that is drawn. The mark is frozen at mount. Offering it as a setting is your call; the panel holds no preference. |
