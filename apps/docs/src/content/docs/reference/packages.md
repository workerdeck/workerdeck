...
| [`@workerdeck/react`](https://www.npmjs.com/package/@workerdeck/react) | The headless React layer: `useClaudeSession` hook + pure transcript reducer. No styling opinion. |
| [`@workerdeck/ui`](https://www.npmjs.com/package/@workerdeck/ui) | The styled agent-control component library: session panel (status bar, streaming transcript, tool-call cards, permission prompts, composer), session list, and the underlying primitives. Tailwind v4 + Base UI + cva; light/dark via tokens. |
