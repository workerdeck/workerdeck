...
- `App/` — the SwiftUI app (hosts, sessions, transcript, permissions, HUD). Hosts + auth keys
  ...
  - `App/Sources/Session/RichTextEditor.swift` — the app's **only UIKit bridge**: a
    `UIViewRepresentable` over `UITextView`. On the 18.0 deployment target SwiftUI can neither
...
- `Widgets/Sources/` — the **widget extension**, which draws the Live Activity and nothing else:
  no credential, no network call. Its only dependency is the `WorkerDeckActivity` product, so the
  transcript reducer and the terminal renderer stay out of a memory-capped process. A card's
  buttons are `LiveActivityIntent`s, performed by the *app* in the background.
- `WorkerDeckKit/Sources/WorkerDeckActivity/` — the Live Activity payload and the intent types,
  shared by the app and the extension. A separate target from `WorkerDeckKit` on purpose: this is a
  contract with the CLI forwarder, not with `packages/protocol`. Its fixtures
  (`Tests/WorkerDeckActivityTests/Fixtures/`) are **generated by
  `packages/cli/test/live-activity.test.ts`** — run that with `UPDATE_FIXTURES=1` after changing
  either side.
...
**iOS 18.0 minimum.** The floor is set by Live Activities: push-to-start — the gateway raising a
card on a phone where the app is not running — needs 17.2, and 18 is what brings the Apple Watch
Smart Stack family and `input-push-token`. Declared in four places that must move together:
`project.yml`'s `options.deploymentTarget.iOS`, its `settings.base.IPHONEOS_DEPLOYMENT_TARGET`,
each target's own `deploymentTarget`, and `WorkerDeckKit/Package.swift`'s `platforms`.
