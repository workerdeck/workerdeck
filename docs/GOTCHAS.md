...
- The model list is **only ever current models**: the older versions Claude Code's own picker
  ...
  is about, and it is why taking a new SDK release is the *only* way a new model reaches us.
  Nothing gates that any more: `pnpm-workspace.yaml` sets `minimumReleaseAge: 0`, because pnpm's
  exclude list could not express codex's platform binaries (see the codex pin bullet).
...
- **A mid-turn message is `turn/steer`, not a queued turn.** `sendMessage` used to push every
  message onto `#turnChain`, so anything typed during a run waited for `turn/completed` before it
  even reached codex: on a long turn that is dozens of minutes, while the claude engine ingests
  the same message into the running turn. `turn/steer {threadId, expectedTurnId, input}` takes the
  same `UserInput` union `turn/start` does, and `expectedTurnId` is a precondition the server
  rejects when it no longer names the active turn. Every rejection falls back to the old queue
  path, so a message is never lost and never sent twice. **Do not detect an unsupported binary by
  `-32601`**: measured against 0.153.4, an unknown method answers `-32600` with
  `Invalid request: unknown variant \`turn/steer\`` (a serde miss on the `ClientRequest` enum),
  and a bad precondition is `-32600` too, so only that literal `unknown variant` message marks a
  connection unable to steer. Steering is allowed while an approval is pending, because the
  approval gates codex's tool call and not the turn's input channel.
...
- `@openai/codex` (the npm package carrying the binary) is pinned to an exact minor
  (`~0.155.1`): pre-1.0, and the JSON-RPC schema regenerates per release. It is an **optional
  peer** of core, whose floor deliberately lags at `~0.149.0` (absent → profiles report
  unavailable, creates throw the install message; no consumer downloads a ~40 MB per-platform
  binary it never uses) and a real dependency of the CLI so the turnkey instance has codex out of
  the box. The runner drives the binary directly, with no SDK in between (`@openai/codex-sdk`
  left with exec; it has no app-server client), resolved two hops through the wrapper package to
  `vendor/<triple>/bin/codex`, the same file the wrapper's own `bin/codex.js` execs. Any change
  to `CodexRunner`'s spawn options, handshake, or event mapping requires a `smoke:codex` run: the
  scripted peer cannot validate the real vocabulary.
...
- **A sub-agent is a SEPARATE THREAD on the same connection, and `threadId` is the only thing
  ...
  `kind` is `started|interacted|interrupted|completed`. It did **not** carry `completed` when
  this engine was written, which is why the child thread's `turn/completed` was the only finish;
  codex added the kind later, and the settle bullet below is the consequence. Codex sends no
  `tool_use` for a spawn either, so the runner authors the anchor call itself as
  `CODEX_AGENT_TOOL`, which is what every agent affordance downstream keys off), and thereafter
  its items, its deltas, its token usage and its own turn lifecycle all arrive on the **same
  JSON-RPC connection**, distinguished only by the `threadId` every notification carries.
  Two consequences, and the first shipped as a bug:
...
- **An agent settles on its `subAgentActivity {kind: 'completed'}` item, not only on its thread's
  ...
  (schema-verified against 0.153.4 and 0.155.1). Settling used to key on the agent thread's own
  `turn/completed`, which `#settleAgentTurn` maps to a `done`/`failed` verdict with the turn's
  report as the anchor's result. That notification is not guaranteed to reach a thread we never
  subscribed to, and when it did not arrive the record stayed `running` forever: the card read
  "processing" for the life of the session, and the `completed` item fell through to the
  revive branch rather than closing anything. Both paths now settle and `settle()` is first-wins,
  so the thread's `turn/completed` still supplies the richer report whenever it arrives first;
  the item-only path closes the row with an empty result, because the item carries no text
  (`{id, kind, agentThreadId, agentPath}` is the whole shape). Pinned by the two ordering tests in
  `core/test/codex-subagents.test.ts`.
