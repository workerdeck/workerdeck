...
xterm, which the shell drill-in needs, **required no CSP change**, and the reason is worth
recording so nobody adds a directive for it. It is an optional peer of `@workerdeck/ui` and a
devDependency here, bundled by this app's own bundler: it code-splits into `dist/webview/chunks/`,
and `cssCodeSplit: false` folds its stylesheet into the single `dist/webview/main.css` that
`style-src ${webview.cspSource}` already covers. The element styles xterm's DOM renderer sets on its
own nodes are covered by the `'unsafe-inline'` that was already in `style-src` for the transcript.
...
`parent.text = view.text`, and on a shell prompt that meant `$ ls` reaching the PTY. Suppressing
...
so the glyph that names the mode is also what undoes it. The rule itself is shared
(`TerminalShell.composerTrigger` in the kit, the mirror of the web composer's): `!ls` is ordinary
text that says `!ls`, the deliberate exit puts the swallowed `$` back in the field
(`TerminalShell.exitDraft`), and backspace on an empty prompt leaves the mode with the field still
empty. The `$` outranks `\u{2715}` in the gutter
...
**Every `$` is a tracked shell**, and the phone mirrors stage 1 of that whole: `ShellInfo` (with
`ShellEndReason` decoding **leniently** to `.unknown`, the one union in the mirror that does -
widening it later would cost a `PROTOCOL_VERSION` bump), `user_message.shell`, the `shell`
transcript item, `promotedShells` for the list, and the four `/sessions/:id/shells` routes on
